...
def worker_num_overhead(algs, topo: TopoGenerator, worker_num_set, switch_num, resources):
    ...
    for num in worker_num_set:
        ...
        aggregation_policy, layer_depolyment=algs[3].run(test_set,resources)
        overhead['ingress']=algs[3].cal_ingress_overhead(test_set, resources, layer_depolyment)
        overhead['in-network']=algs[3].cal_innetwork_aggregation_overhead(test_set, resources, aggregation_policy)
        overhead['total']=algs[3].cal_total_overhead(test_set, resources, aggregation_policy, layer_depolyment)
