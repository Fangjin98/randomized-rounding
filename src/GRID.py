...
class GRID(BasicAlg):
    ...
    def run(self, test_set, comp, band, solver=None):
        ...
        aggregation_node = self._random_rounding(optimal_results, ps, worker_set, switch_set)

        sending_rate = dict()
        switch = {s: 0 for s in switch_set}
        count = 0  # the number of switches plus the number of workers aggregated by the ps
